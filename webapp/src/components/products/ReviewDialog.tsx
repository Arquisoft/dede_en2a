...
import {addReview, getReviewsByCodeAndWebId, modifyReview} from "../../api/api";
...
export default function ReviewDialog(props: ReviewDialogProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const oldReviews: Review[] = await getReviewsByCodeAndWebId(props.product.code, props.webId);
        ...
        if (props.webId === "") {
            ...
            if (reviews.length > 0) {
                ...
                if (await modifyReview(createdReview)) {
                    ...
                    sendNotification("error", "There was an error modifying your review!");
            } else {
